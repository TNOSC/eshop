...
public partial class CheckoutPage : ComponentBase
{
    ...
    private async Task LoadAsync()
    {
        ...
        ApiResult<BasketDto> basketResult = await BasketApi.GetBasketAsync(cancellationToken: CancellationToken.None);
        ApiResult<Customer> customerResult = await IdentityApi.GetMeAsync(cancellationToken: CancellationToken.None);
    ...
    private async Task PlaceOrderAsync()
    {
        ...
        try
        {
            ApiResult<Guid> result = await OrderingApi.PlaceOrderAsync(
            ...
            if (result.IsSuccess)
            {
                ...
                Navigation.NavigateTo(uri: $"/orders/{result.Value}?placed=true");
                return;
            }
