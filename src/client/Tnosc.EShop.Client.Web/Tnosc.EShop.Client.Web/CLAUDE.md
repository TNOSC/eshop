...
2. **Interactive Server render** of the `.Client` project's components (plus WebAssembly render mode
   for the same components) — see the sibling `Tnosc.EShop.Client.Web.Client/CLAUDE.md` for
...
[`.claude/rules/authorization.md`](../../../../.claude/rules/authorization.md) for the same reason
...
- [ ] New settings go through an `Options` class per `.claude/rules/configuration-options.md`
