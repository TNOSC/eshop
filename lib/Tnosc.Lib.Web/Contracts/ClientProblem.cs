...
/// </summary>
public sealed record ClientProblem(
    string? Type,
    string? Title,
    int? Status,
    string? Detail,
...
{
    /// <summary>
    /// Builds a fallback <see cref="ClientProblem"/> for a non-success response whose body could not be
    /// parsed as ProblemDetails at all.
    /// </summary>
    /// <param name="status">The HTTP status code of the response.</param>
    public static ClientProblem FromStatus(int status) =>
        new(
            Type: null,
            Title: null,
            Status: status,
