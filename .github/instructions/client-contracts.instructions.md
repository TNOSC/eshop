---
description: "Client.Web.Contracts: the DTOs shared between the BFF and the Blazor client"
applyTo: "src/client/Tnosc.EShop.Client.Web/Tnosc.EShop.Client.Web.Contracts/**"
---
...
sides of the wire are independent, matched only by convention (mirroring how `authorization.instructions.md`
...
- **One `ApiRoutes.<Context>` nested class per bounded context**, mirroring the server's own
  `<Context>Routes` constants (`authorization.instructions.md` neighbors,
