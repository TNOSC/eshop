...
argument-hint: "[optional: suite to limit to, e.g. \"architecture\" or \"unit\"]"
---

Run this repo's definition of done and report what actually happened.

Optional `$ARGUMENTS` limits the run to one suite (`build`, `architecture`, `unit`, `integration`).
With no arguments, run everything.
...
2. **Architecture tests.** `dotnet test tests/server/Tnosc.EShop.Server.Tests.Architecture`
   ...
   Never suggest relaxing the test as the fix (`.claude/rules/analyzer-suppressions.md`).
