...
reads like a permissions bug rather than a spelling mistake. This is exactly the `cache-tags.md`
...
`IOptions<T>` are touched, per `configuration-options.md`, which `ConfigurationTests` enforces against
